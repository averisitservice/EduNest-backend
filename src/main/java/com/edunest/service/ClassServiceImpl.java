...
public class ClassServiceImpl implements ClassService {
    ...
    public List<ClassListResponse> getClassList(Integer tenantId) {
        ...
        for (ClassMaster classMaster : classes) {
            List<ClassSection> classSections = classSectionRepository
            ...
            if (currentYear != null) {
                classFee = classFeeRepository.findByClassIdAndAcademicYearIdAndTenantId(
                        classMaster.getClassId(), currentYear.getAcademicYearId(), tenantId);
            }
    ...
    public ClassRequest getClassById(Integer classId, Integer tenantId) {
        ...
        List<ClassSection> classSections = classSectionRepository
                .findByClassIdAndTenantId(classId, tenantId);
        List<String> sectionNames = new ArrayList<>();
        for (ClassSection cs : classSections) {
            sectionNames.add(cs.getSectionName());
        }
        ...
        if (currentYear != null) {
            classFee = classFeeRepository.findByClassIdAndAcademicYearIdAndTenantId(
                    classId, currentYear.getAcademicYearId(), tenantId);
        }
    ...
    public boolean saveClass(Integer classId, Integer tenantId, ClassRequest request) {
        ...
        if (isEdit) {
            classMaster = classMasterRepository.findById(classId)
                    .orElseThrow(() -> new CustomException("classId", "Class not found"));
        ...
        if (request.getSections() != null && !request.getSections().isEmpty()) {
            if (isEdit) {
                List<ClassSection> oldSections = classSectionRepository
        ...
        if (request.getAnnualFee() != null) {
            ClassFee classFee = classFeeRepository
                    .findByClassIdAndAcademicYearIdAndTenantId(savedClassId, academicYearId, tenantId);
